...
ClayMan::ClayMan(
    ...
    assert(
        claymaninstancehasbeencreated == false &&
        "Only One Instance of ClayMan Should be Created!"
    );
    if (windowWidth == 0) {
        windowWidth = 1;
    }
    ...
    Clay_Initialize(clayMemory, d, (Clay_ErrorHandler)handleErrors);
...
void ClayMan::updateClayState(
    ...
    Clay_SetLayoutDimensions(d);

    Clay_Vector2 v = {mouseX, mouseY};
    Clay_SetPointerState(v, leftButtonDown);

    Clay_Vector2 v2 = {scrollDeltaX, scrollDeltaY};
    Clay_UpdateScrollContainers(true, v2, frameTime);
}
...
Clay_Sizing ClayMan::fixedSize(const uint32_t w, const uint32_t h) {
    return {
        .width = (Clay_SizingAxis{
            .size = {.minMax = {(float)w, (float)w}},
            .type = CLAY__SIZING_TYPE_FIXED
        }),
        .height = (Clay_SizingAxis{
            .size = {.minMax = {(float)h, (float)h}},
            .type = CLAY__SIZING_TYPE_FIXED
        })
    };
}
...
Clay_Sizing ClayMan::expandXY() {
    return {
        .width = (Clay_SizingAxis{
            .size = {.minMax = {}}, .type = CLAY__SIZING_TYPE_GROW
        }),
        .height = (Clay_SizingAxis{
            .size = {.minMax = {}}, .type = CLAY__SIZING_TYPE_GROW
        })
...
bool ClayMan::mousePressed() {
    return Clay_GetCurrentContext()->pointerInfo.state ==
...
void ClayMan::applyElementConfigs(Clay_ElementDeclaration& configs) {
    ...
    Clay__ConfigureOpenElement(
        (Clay__Clay_ElementDeclarationWrapper{configs}).wrapped
    );
}
